...
class Scraper
  ...
  private
    ...
    def add_general_data(pokemon)
      ...
      type_1 = "td:nth-child(3) > a:nth-child(1)"
      type_2 = "td:nth-child(3) > a:nth-child(3)"
      image_link = "td:nth-child(1) > span:nth-child(1)"

      {
        original_id: css_text(pokemon, id),
        name: css_text(pokemon, name, false) ,
        type_1: css_text(pokemon, type_1, false),
        type_2: css_text(pokemon, type_2, false),
        img_link: pokemon
          .css(image_link)&.children.first&.attributes["data-src"].value || nil,
      }
    end
    ...
    def add_stat_data(pokemon)
      css_selector = lambda { |child|  "td:nth-child(#{child})"}
      {
